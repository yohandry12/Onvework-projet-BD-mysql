...
import { CheckCircleIcon, ClockIcon, XCircleIcon, BriefcaseIcon, QuestionMarkCircleIcon } from "@heroicons/react/outline";
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 border hover:shadow-lg">
      <div className="flex justify-between">
        ...
        <div className={`flex items-center gap-2 px-3 py-1 rounded-full h-fit text-sm ${currentStatus.bgColor} ${currentStatus.textColor}`}>
          {currentStatus.icon}
      ...
      <div className="flex justify-end gap-3 mt-4">
        {status === "pending" && (
          <button onClick={handleWithdraw} className="px-4 py-2 text-sm font-semibold text-red-700 bg-red-100 rounded-lg hover:bg-red-200">
            Retirer
          </button>
        )}
        {/* CORRIGÉ: Utiliser job.id */}
        <Link to={`/jobs/${job.id}`} className="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700">
          Voir l'offre
        </Link>
      </div>
    </div>
...
const MyApplications = () => {
  ...
  const otherApps = applications.filter((app) => !["accepted", "pending"].includes(app.status));
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {applications.length === 0 ? (
        <div className="text-center bg-white p-10 rounded-lg shadow-sm">
            ...
            <div className="mt-6"><Link to="/jobs" className="px-5 py-2.5 bg-blue-600 text-white rounded-lg">Trouver des missions</Link></div>
        </div>
      ) : (
        <div className="space-y-12">
          {acceptedApps.length > 0 && <section>
            <h2 className="text-2xl font-semibold ...">Missions Acceptées</h2>
            <div className="grid gap-6">
              {acceptedApps.map((app) => ( <ApplicationCard key={app.id} application={app} onWithdraw={handleApplicationWithdrawn} /> ))}
            </div>
          </section>}
          
          {pendingApps.length > 0 && <section>
            <h2 className="text-2xl font-semibold ...">Candidatures en Attente</h2>
            <div className="grid gap-6">
              {pendingApps.map((app) => ( <ApplicationCard key={app.id} application={app} onWithdraw={handleApplicationWithdrawn} /> ))}
            </div>
          </section>}
          
          {otherApps.length > 0 && <section>
            <h2 className="text-2xl font-semibold ...">Historique</h2>
            <div className="grid gap-6">
              {otherApps.map((app) => ( <ApplicationCard key={app.id} application={app} onWithdraw={handleApplicationWithdrawn} /> ))}
            </div>
          </section>}
        </div>
      )}
