...
const { User, CandidateProfile, ClientProfile, sequelize } = require("../models"); // Import de tous les modèles + l'instance sequelize
...
router.post("/register", async (req, res) => {
    ...
    try {
        const { email, password, role = 'candidate', firstName, lastName,  company, employerType, phone, location, sector, commercialName, associationName } = req.body;
        ...
        if (role === 'candidate') {
            ...
            await ClientProfile.create({
                userId: user.id, firstName, lastName, company, employerType, phone, location, sector, commercialName, associationName
            }, { transaction: t });
        }
...
router.post("/login", async (req, res) => {
    try {
        ...
        const fullUser = await User.findByPk(user.id, {
            include: user.role === 'candidate' ? 'candidateProfile' : (user.role === 'client' ? 'clientProfile' : undefined)
        });
...
router.post("/admin/login", async (req, res) => {
  try {
      ...
      if (!adminUser || !(await adminUser.matchPassword(password))) {
          ...
          return res.status(401).json({ success: false, error: "Identifiants d'administrateur incorrects." });
      }
...
router.get("/me", authenticateToken, async (req, res) => {
    try {
        ...
        const fullUser = await User.findByPk(req.user.id, {
            include: req.user.role === 'candidate' ? 'candidateProfile' : (req.user.role === 'client' ? 'clientProfile' : undefined)
        });
...
router.put('/profile', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!profileUpdates || typeof profileUpdates !== 'object') {
        return res.status(400).json({ success: false, error: 'Données de profil invalides.' });
    }
    ...
    if (!profileToUpdate) {
        if (user.role === 'candidate') {
            profileToUpdate = await CandidateProfile.create({ userId: user.id, ...profileUpdates }, { transaction: t });
        } else if (user.role === 'client') {
            profileToUpdate = await ClientProfile.create({ userId: user.id, ...profileUpdates }, { transaction: t });
        }
    } 
    // Si le profil existe déjà, on le met à jour
      ...
      for (const field of jsonFields) {
          ...
          if (cleanedUpdates[field] !== undefined) {
              ...
              if (typeof cleanedUpdates[field] === 'string' && cleanedUpdates[field].trim() !== '') {
                  try {
                      ...
                      return res.status(400).json({ success: false, error: `Le champ '${field}' n'est pas un JSON valide.` });
                  }
              }
              // Si ce n'est pas déjà un objet/tableau, on s'assure que c'est bien `null` ou un format valide
              else if (typeof cleanedUpdates[field] !== 'object' && cleanedUpdates[field] !== null) {
